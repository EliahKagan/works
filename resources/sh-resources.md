...
than people tend to think, even experienced people, even experienced people
who are trying to account for how tricky it is. Most unofficial guides, and
even many unofficial (yet reputably published) books, are bad. I recommend the
...
are not subshells, only get environment variables, i.e., exported variables.
A variable that is not inherited as an environment variable, and that is not
...
features. Note also that this is NOT the same document as the manpage
("man bash"), though that is also somewhat useful. I recommend anyone who
writes Bash scripts regularly, or any Bash scripts where quality is important,
read this all the way through at least once (in order or otherwise). If at
first you don't grok it, you may want to use sources on Greg's wiki first and
...
far less so than many language standards. I recommend reading or at least
...
In that vein, you may want to start with "Why is using a shell loop to process
...
use it. Although there's quite a bit that can be done in "pure shell," at least
with feature-rich shells like Bash, this is an unidiomatic and typically
inferior way to write shell scripts. "Knowing" shell scripting means knowing
how to use common external commands like those mentioned above, though for
language interpreters that is not necessarily the same as really knowing them
as whole languages. In effect, the "standard library" of the shell command
language consists of all the other utilities that POSIX requires to be present.
The POSIX toolset is big enough that it is easy to get dispirited about
knowledge one doesn't (or doesn't yet) possess, but remember that even partial
proficiency goes a long way. That shells are glue languages mandates
familiarity with standard external commands but also makes it far less
